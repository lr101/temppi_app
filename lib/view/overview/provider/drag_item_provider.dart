...
class DragItem extends _$DragItem {
  ...
  List<SensorDto> build() {
    return ref.watch(sensorListProvider).when(
      data:  (cb) {
        ...
        final list = ref.watch(sharedPreferencesProvider).getStringList("SENSOR_DRAG_LIST") ?? [];
        for (String sensorId in list) {
          final sensor = newItems.where((element) => element.sensorId == sensorId);
        ...
        ref.read(sharedPreferencesProvider).setStringList("SENSOR_DRAG_LIST", returnList.map((e) => e.sensorId!).toList());
        return returnList;
      },
      error: (err, stack) => [],
      loading: () => [],
  ...
  void update(List<SensorDto> list) {
    state = [...list];
    ref.read(sharedPreferencesProvider).setStringList("SENSOR_DRAG_LIST", state.map((e) => e.sensorId!).toList());
  }
}